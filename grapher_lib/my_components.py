...
class Tutorials:
    grafikas_content = dbc.Card(
        dbc.CardBody(
            [
                ...
                html.Div(children=[
                    html.P("Grafike atvaizduojami pasirinktų lentelių ryšiai, kurie yra aprašomi užklausos faile."),
                    html.P("Grafikas yra interaktyvus: "),
                    html.P("Grafiko vaizdą galima pritraukti/atitraukti"),
                    html.P("Taškus galima tampyti pavieniui arba kelis (reikia nuspausti CTRL)"),
                    html.P(
    ...
    grafikas = html.Div([
        dbc.Button("Grafiko instrukcija", id="tutorial-grafikas-legacy-target", color="success", n_clicks=0),
        dbc.Popover(grafikas_content, target="tutorial-grafikas-legacy-target", body=True, trigger="legacy")
    ])
        ...
        dbc.CardBody(
            [
                ...
                html.P(children=[
                    ...
                    html.P(children=[html.B("Layout - "), html.Label("Grafiko taškų išdėstymo schemos.")]),
                    html.P(children=[html.B("Select tables to graph - "),
                                     html.Label("pasirinkite kurių PDSA lentelių ryšius norite atvaizduoti")]),
                    html.P(children=[html.B("Add list of tables - "), html.Label(
                        "galima nurodyti atvaizduoti sąrašą lentelių. Sąraše lentelių pavadinimai turi būt atskirti kableliais")]),
                    html.P(children=[html.B("Get neighbours - "), html.P(
                        "Grafiką papildo lentelėmis, kurios tiesiogiai siejasi su pasirinktomis lentelėmis")]),
                    html.P(children=[html.B("Get info about columns of selected tables - "),
                                     html.Label("Parodo informaciją apie pasirinktos lentelės(-ių) stulpelius")]),
                    html.P(children=[html.B("Get info on displayed tables - "),
                                     html.Label("parodo grafike atvaizduotų lentelių informaciją")]),
                ], className="card-text")
            ]),
        # style={"width": "50%"}
    )
    filtrai = html.Div([
        dbc.Button("Filtrų instrukcija", id="tutorial-filtrai-legacy-target", color="success", n_clicks=0),
        dbc.Popover(filtrai_content, target="tutorial-filtrai-legacy-target", body=True, trigger="legacy")
    ])
