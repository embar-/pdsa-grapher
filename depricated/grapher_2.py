...
df = gu.get_data_for_network(pdsa)
...
def get_compatable_data(df_data):
    my_network_data = df_data.loc[:,['table', 'column']].apply(lambda x: '{} {}'.format(x.iloc[0], x.iloc[1]), axis=1)
    my_network_data = my_network_data.tolist()
    return my_network_data
...
from grapher_lib.foreign_libs.demos import dash_reusable_components as drc
import os
...
# ###################### DATA PREPROCESSING ######################
# Load data
# with open('grapher_lib/foreign_libs/demos/data/sample_network.txt', 'r') as f:
#     network_data = f.read().split('\n')

network_data = get_compatable_data(df)
